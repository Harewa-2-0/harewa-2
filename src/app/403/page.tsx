...
function UnauthorizedContent() {
  ...
  const reason = useMemo(
    () => search.get('reason') || 'This area is for administrators.',
    [search]
  );
  ...
  useEffect(() => {
    ...
    // If an admin somehow hits /403 (user page), send them to the admin variant
    if (user && user.role === 'admin' && !on403) {
      router.replace('/403/admin');
      return;
    }
        ...
        <div className="relative rounded-2xl bg-white/90 backdrop-blur-xl shadow-xl ring-1 ring-black/5 border border-gray-200">
          ...
          <div className="p-8 md:p-10 text-center">
            ...
            <p className="mt-3 text-gray-600 leading-relaxed">
              {reason} You're signed in as a <span className="font-medium">user</span>.
              {from && (
                <>
                  {' '}You tried to open{' '}
